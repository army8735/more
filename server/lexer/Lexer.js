var Class = require('../util/Class'),
	...
	Lexer = Class(function(rule) {
		...
		this.code; //要解析的代码
		this.peek = ''; //向前看字�?
		this.index = 0; //向前看字符字符索�?
		this.isReg = Lexer.IS_REG; //当前/是否是perl风格正则表达�?
		this.lanDepth = 0; //生成�?��结果时需要记录的行深�?
		this.tokenList = []; //结果的token列表
		this.parentheseState = false; //(�?��时标记之前终结符是否为if/for/while等关键字
		this.parentheseStack = []; //圆括号深度记录当前是否为if/for/while等语句内�?
		this.cacheLine = 0; //行缓存�?
		this.totalLine = 1; //总行�?
		this.colNum = 0; //�?
		this.colMax = 0;
	}).methods({
